...
| display   | optional  | `string` | Return Result: By default, returns proxy:port<br />display=ip\_port\_user\_pass,it returns proxy,port,username,and password<br />display=ip\_port\_user\_pass\_loc,it returns proxy,port,username,password,and region|
