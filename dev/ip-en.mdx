...
### Request by continent
- For example, use an IP in Americas area and set the IP replacement cycle to 1 minute.
- Forward by continent (cont): Americas (nasa), Asia (as), Europe (eu), Africa (af), Oceania (oa)
- You can set it in the program in the following ways：
`{username}-cont-{nasa}-period-{1}:{password}`, fill in the user name, cont code, IP replacement cycle (minutes), and password. For example, username `gf2814951572`, password`aft415oyt`, used in this request`Americas area ip、The IP address replacement interval is 1 minute`。
